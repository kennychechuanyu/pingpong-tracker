<script>
  ...
  import { getDanRank, getEarnedSeals, SEALS } from '../lib/training.js'
  ...
  // 修行 Training Log
  $: danInfo = getDanRank(total)
  $: earnedSeals = player ? getEarnedSeals(player, sorted) : []
  $: earnedSealIds = new Set(earnedSeals.map(s => s.id))
      ...
      <div class="card training-card">
        <div class="training-header">
          <div class="training-title">
            <span class="training-zh">修行</span>
            <span class="training-en">Training Log</span>
          </div>
          <div class="training-sub">volume tracks separately from Elo</div>
        </div>

        <div class="dan-block">
          {#if danInfo.current}
            <div class="dan-badge">
              <span class="dan-zh">{danInfo.current.zh}</span>
              <span class="dan-en">{danInfo.current.en}</span>
            </div>
          {:else}
            <div class="dan-badge dan-unranked">
              <span class="dan-zh">未段</span>
              <span class="dan-en">Unranked</span>
            </div>
          {/if}
          <div class="dan-progress">
            {#if danInfo.next}
              <div class="dan-progress-top">
                <span class="dan-progress-label">
                  next: <strong>{danInfo.next.zh}</strong> <span class="dan-progress-en">{danInfo.next.en}</span>
                </span>
                <span class="dan-progress-count tnum">{danInfo.matchesToNext} to go</span>
              </div>
              <div class="dan-bar-track">
                <div class="dan-bar-fill" style="width: {(danInfo.progress * 100).toFixed(1)}%"></div>
              </div>
            {:else}
              <div class="dan-max">十段 reached — 名人 Master</div>
            {/if}
          </div>
        </div>
        ...
        <div class="seals-header">
          <span class="seals-label">
            <span class="seals-zh">印章</span>
            <span class="seals-en">Seals</span>
          </span>
          <span class="seals-count tnum">{earnedSeals.length} / {SEALS.length}</span>
        </div>
        <div class="seals-grid">
          {#each SEALS as seal}
            {@const earned = earnedSealIds.has(seal.id)}
            <div
              class="seal"
              class:seal-earned={earned}
              class:seal-locked={!earned}
              title="{seal.en} — {seal.desc}"
            >
              <span class="seal-zh">{seal.zh}</span>
              <span class="seal-en">{earned ? seal.en : '— — —'}</span>
            </div>
          {/each}
        </div>
      </div>
...
<style>
  ...
  .training-card {
    background:
      radial-gradient(ellipse at top left, rgba(196,30,30,0.08), transparent 55%),
      var(--surface);
  ...
  .dan-badge {
    ...
    gap: 2px;
    box-shadow:
      0 2px 10px rgba(196,30,30,0.25),
      inset 0 1px 0 rgba(255,255,255,0.08);
  }

  .dan-badge.dan-unranked {
  ...
  .dan-bar-fill {
    height: 100%;
    background: linear-gradient(90deg, #c41e1e, #e8a04a);
    border-radius: 3px;
    transition: width 0.4s ease;
    box-shadow: 0 0 8px rgba(232,160,74,0.35);
  }
  ...
  .seal-earned {
    background:
      radial-gradient(ellipse at center, #d42a2a 0%, #8a0f0f 80%),
      #8a0f0f;
    border: 2px solid #5a0505;
    color: #fdf4e3;
    box-shadow:
      0 2px 6px rgba(196,30,30,0.35),
      inset 0 0 0 1px rgba(255,230,200,0.15);
    transform: rotate(-2deg);
  }

  .seal-earned:nth-child(3n) { transform: rotate(1.5deg); }
  .seal-earned:nth-child(4n) { transform: rotate(-1deg); }
  .seal-earned:nth-child(5n) { transform: rotate(2deg); }
